...
static __inline void wideMultiply(rep_t a, rep_t b, rep_t *hi, rep_t *lo) {
  ...
  const rep_t plolo = loWord(a) * loWord(b);
  const rep_t plohi = loWord(a) * hiWord(b);
  const rep_t philo = hiWord(a) * loWord(b);
  const rep_t phihi = hiWord(a) * hiWord(b);
  // Sum terms that contribute to lo in a way that allows us to get the carry
  const rep_t r0 = loWord(plolo);
  const rep_t r1 = hiWord(plolo) + loWord(plohi) + loWord(philo);
  *lo = r0 + (r1 << 32);
