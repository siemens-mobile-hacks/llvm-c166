; RUN: llc -mtriple=c166-none-elf < %s | FileCheck %s
; RUN: llc -mtriple=c166-none-elf -filetype=obj < %s -o %t.o
; RUN: llvm-readobj --file-header %t.o | FileCheck %s --check-prefix=OBJ
; RUN: llvm-readobj --relocations %t.o | FileCheck %s --check-prefix=RELOC
; RUN: llc -mtriple=c166-none-elf -stop-before=c166-asm-printer < %s | FileCheck %s --check-prefix=MIR
...
;
; The Large model passes argument words through R12-R15 without
; imposing even-register alignment on a two-word argument.  Scalar return
; values use R4 and functions return with RETS.
;
; OBJ:      Format: elf32-c166
; OBJ:      Machine: EM_C166 (0x74)
; OBJ:      Flags [ (0x121)
; OBJ-DAG:    EF_C166_CODE_HUGE (0x100)
; OBJ-DAG:    EF_C166_CORE_8X166 (0x1)
; OBJ-DAG:    EF_C166_DATA_FAR (0x20)
; RELOC:      R_C166_SEG8 _callee_mix
; RELOC-NEXT: R_C166_SOF16 _callee_mix
; MIR-LABEL:  name: forward_mix
; MIR:        CALLS @callee_mix, @callee_mix, $r12, $r14r13, $r15, csr_c166_callpreserved
...
define i16 @load_far(ptr %address) {
...
define void @store_far(ptr %address, i16 %value) {
; CHECK-LABEL: _store_far:
; CHECK:       extp r13, #1
; CHECK-NEXT:  mov [r12], r14
; CHECK-NEXT:  rets
  store i16 %value, ptr %address, align 2
  ret void
}
...
; CHECK:       calls seg(_callee_mix), sof(_callee_mix)
; CHECK-NEXT:  rets
...
; CHECK-NEXT:  calls seg(_callee_mix), sof(_callee_mix)
; CHECK-NEXT:  rets
...
; CHECK:       calls seg(_callee_word), sof(_callee_word)
; CHECK-NEXT:  rets
...
; CHECK:       calls seg(_callee_byte), sof(_callee_byte)
; CHECK-NEXT:  rets
...
; CHECK-NEXT:  calls seg(_callee_five_words), sof(_callee_five_words)
; CHECK-NEXT:  add r0, #2
; CHECK-NEXT:  rets
...
; CHECK-NEXT:  calls seg(_callee_stack_stop), sof(_callee_stack_stop)
; CHECK-NEXT:  add r0, #6
; CHECK-NEXT:  rets
...
; CHECK:       calls seg(_callee_packed3), sof(_callee_packed3)
; CHECK:       add r0, #6
; CHECK-NEXT:  rets
