...
; The relaxation passes preserve short local branches and use conditional
; same-segment jumps for out-of-range edges within a function.
define i16 @wide_switch(i16 %value) {
; CHECK-LABEL: wide_switch:
; CHECK: jmpr
; CHECK: jmpa cc_{{[a-z]+}}, cof(.LBB{{[0-9_]+}})
entry:
...
; A register-bit branch is four bytes rather than two, but it has the same
; signed eight-bit word displacement.  Keep the common path in line and place
; the unlikely destination beyond a wide switch so BranchRelaxation has to
; invert the bit test and insert JMPS. JMPA cannot directly test a register
; bit, so this form deliberately remains a two-instruction sequence.
define i16 @wide_bit_branch(i16 %value) {
; CHECK-LABEL: wide_bit_branch:
; CHECK: jnb r{{[0-9]+}}.15, [[NEXT:.LBB[0-9_]+]]
; CHECK: jmps seg([[FAR:.LBB[0-9_]+]]), sof([[FAR]])
; CHECK: [[NEXT]]:
entry:
  %masked = and i16 %value, -32768
  %set = icmp ne i16 %masked, 0
  br i1 %set, label %bitset, label %dispatch, !prof !0
