// RUN: %clang_cc1 -triple c166-unknown-none -mcmodel=large -O2 -emit-llvm -o - %s | FileCheck %s --check-prefix=IR
// RUN: %clang_cc1 -triple c166-unknown-none -mcmodel=large -O2 -S -o - %s | FileCheck %s --check-prefix=ASM
// RUN: %clang_cc1 -triple c166-unknown-none -mcmodel=large -O0 -S -o /dev/null -mllvm -verify-machineinstrs %s
// RUN: %clang_cc1 -triple c166-unknown-none -mcmodel=large -O2 -S -o /dev/null -mllvm -verify-machineinstrs %s
// RUN: %clang_cc1 -triple c166-unknown-none -mcmodel=medium -O1 -emit-obj -o %t.medium.o %s
// RUN: %clang_cc1 -triple c166-unknown-none -mcmodel=small -O1 -emit-obj -o %t.small.o %s
