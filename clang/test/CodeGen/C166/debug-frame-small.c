// RUN: %clang_cc1 -triple c166-none-elf -mcmodel=small -O1 \
// RUN:   -debug-info-kind=limited -main-file-name debug-frame-small.c \
...
// RUN:   --section-start=.text=0x180000 -e _small_far %t.o -o %t
...
// Small changes the default data-pointer class, not the default function
// class. Its ordinary function therefore keeps the far CALLS/RETS
// system-stack frame, while an explicitly near function uses the two-byte
// near return frame.  Verify both after LLD has resolved their FDE addresses.

typedef unsigned int u16;

u16 small_far(u16 value) {
  volatile u16 slot = value;
  return slot + 1U;
}

u16 __attribute__((c166_near)) small_near(u16 value) {
  volatile u16 slot = value;
  return slot + 2U;
}
